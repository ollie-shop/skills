...
| **7. Function authoring** | "hub function", "middleware", "external validation", "call an API", "intercept", "rewrite request", "resolver", "enricher", "enforce", "session storage", "cookie" | `references/hub-functions.md` + `references/patterns/resolver-enricher-enforce-with-cookies.md` (if multi-function pattern) + the target function's `INSTRUCTIONS.md` |
