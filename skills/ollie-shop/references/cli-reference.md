...
Read this before the first deploy of a **component**. Everything in this section is about the CLI's bundler, which is what the Studio deploy button (it fetches `/bundle` from the local CLI) and `ollieshop deploy` both go through. It does **not** apply to the function path below: there the developer builds the zip by hand and uploads it through the admin form, so an `.ollieignore` has no say in what ships.

**For a component deploy, the bundle is the whole project root, not just the component being deployed.**
