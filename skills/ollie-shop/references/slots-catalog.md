...
> **Two-step modal.** `modal_shipping_options` and `modal_shipping_address` are the two steps of the same modal and only one mounts at a time. The step lives in the host: calling `onBackToAddress()` from the options step swaps to the address step, and calling `onSubmit(address)` from the address step swaps back once the address resolves. Override them independently — a custom component in one step keeps the default UI in the other.
>
