...
| `{{ summary }}_summary` | The step's `summary` value, lowercased — **not** its id. `summary: "Contact"` emits `contact_summary`. | Replace the collapsed summary row shown above the active step. A custom step gets one too: declare `summary` on it in the version props and register a component on the resolved id. |
...
| `cart_item_details` | Empty by default. Renders once per item between the product title and its unit price, with `item` prop. Good place for badges/tags or other per-item content. |
| `card_item_base` | Base wrapper around an item card. |
| `cart_items_package_list_item` | Whole shipping-package card on the cart (header + items). Mounts once per package with `shippingPackage`, `itemsInPackage` and `itemsCountInPackage`. Rendering nothing hides that package. |
...
| `cart_digital_banner` | Info banner above the digital package group on the cart ("immediate delivery by email"). Only mounts when digital packages exist. |
...
| `cart_mobile_summary` | Wrapper around the mobile-only cart footer (coupon form + totalizer). Groups the region so it can be overridden as a unit. |
...
| `checkout_mobile_summary` | Wrapper around the mobile-only summary footer of the payment step (contains the totalizer). Groups the region so it can be overridden as a unit. |
...
| `contact_summary_modal` | Wraps the contact-details edit form ("Detalii de contact") as `children`. Renders around the profile form inside the edit modal — good place for legal/compliance notices or an extra field. Reaches customer/session via `useCheckoutSession()`. |
| `shipping_summary` | Collapsed shipping summary row. |
| `{{ summary }}_summary` | **Dynamic**. One slot per step that defines a `summary`, custom steps included. See "Dynamic slots" above. |

## Shipping

| Slot ID | Notes |
|---|---|
| `shipping_address_autocomplete` | Address input shown right after the email (postal code or street autocomplete, plus the mobile "use my location" button). Receives `onSubmit(address)`, `isLoading`, `error`, and the postal-code step: `isAskingPostalCode`, `pendingAddress`, `onEditAddress()`. See the note below. |
...
| `shipping_list_packages` | Package list area. Receives `type` (`delivery` | `pick_up` | `digital`). The `digital` instance only mounts when a custom function marks packages as digitally fulfilled. |
| `shipping_package_shipping_options` | Per-package shipping option picker. |
| `shipping_package_shipping_options_item` | One row of that picker, rendered once per quote. Receives `option` (the quote), `selectedPackage`, `isSelected`, `price`, `label`, `formatPrice`, `onSelect()`, `disabled`, `isLoadingOptions`. `children` is the native row, so returning it for the quotes you don't care about keeps the default. |
...
| `shipping_pickup_receiver_form` | Form that opens on a pickup package to name someone else to collect ("Responsável pela retirada"). Receives `receiver` (`{ firstName, lastName }` as saved), `onSaveReceiver(receiver)` (saves it as the pickup address's receiver name), `inputs` (the native fields' props) and `packageSelected`. One per pickup package; the checkbox above it stays native. |
...
> **Custom autocomplete on `shipping_address_autocomplete`.** `onSubmit(address)` is the native request: it picks the right action, fills in country and address type, drives `isLoading`, and failed submits still show as messages. Search with `useAddress()` (`searchPredictions`, `predictions`) and resolve a pick with `selectPrediction(id)` before submitting, since `onSubmit` does not resolve Places ids.
>
> An address without a postal code, in a country that requires one, is not sent. Instead `isAskingPostalCode` turns `true` and `pendingAddress` holds the address. What happens next depends on `children`:
>
> - **Augment** (render `{children}`): the default UI switches to its postal-code field by itself.
> - **Replace** (own layout): render your own postal-code field while `isAskingPostalCode` is `true`, submit `onSubmit({ ...pendingAddress, postalCode })`, and call `onEditAddress()` to go back to the search. `useAddress().lookupByPostalCode(postalCode, country)` fills in city and state. Ignoring `isAskingPostalCode` leaves the shopper on a submit that does nothing.
...
| `modal_new_address_form` | The whole "new address" form opened from the delivery address modal, Back and Save buttons included. Same props as `modal_edit_address_form`, with `mode: "new"` and `address` as the optional prefill. |
| `modal_edit_address_form` | The whole "change address" form for a saved address inside the delivery address modal, Back and Save buttons included. Receives `mode` (`new` \| `edit`), `address`, `addressForm`, `onSubmit(address)`, `isSubmitting`, `onBack`, `onClose`. `onSubmit` runs the host's save without validating (use `addressForm.validate()` / `getValues()` for the native rules) and only persists address fields: an extra field must be stored by the component itself. In edit mode the values are written over the edited address, so its id is kept. |
| `modal_complete_address_form` | The whole "complete the address" form of the delivery step — street summary with the "Change" link, number field with the "no number" checkbox, complement field and Save button. Receives `address`, `onSubmit(values)`, `isSubmitting`, `onChange`, `onClose`, `withoutNumberValue`. `onSubmit` writes the values over the address being completed, so its id is kept, and runs the host's save without validating. `onChange` opens the address selection flow. Only mounts when the `completeAddressModal` store flag is on and the selected delivery address is missing its number. |
| `modal_shipping_address` | Address step of the "Calculate shipping" modal (postal code / autocomplete input). Receives `onSubmit`, `isSubmitting`, `error`. |
| `modal_shipping_options` | Quote step of the "Calculate shipping" modal (address summary + quote list + save button). Receives `onBackToAddress` to go back to `modal_shipping_address`, plus `selectedQuote`, `onSelectQuote`, `onSave`. |
| `cart_modal_shipping_item` | One row of the item list reached from the "+N" on the quote step's thumbnail strip. Receives `item`; one instance per item covered by the selected quote. |

> **Steps of one modal.** `modal_shipping_options` and `modal_shipping_address` are steps of the same modal and only one mounts at a time. The modal has other steps (time picker, pickup point list, item list) that it turns into in place rather than stacking; of those, only the item list is overridable, through `cart_modal_shipping_item`. The step lives in the host: calling `onBackToAddress()` from the options step swaps to the address step, and calling `onSubmit(address)` from the address step swaps back once the address resolves. Override them independently — a custom component in one step keeps the default UI in the other.
>
> Props carry only modal-local state. The quotes, the selected address and the locale used to format prices come from `useCheckoutSession()`; the saving state of `onSave` comes from `usePendingActions()` as `pendingActions.includes("UPDATE_SHIPPING_PACKAGES")`. Only `isSubmitting` on `modal_shipping_address` stays a prop — that step runs a custom server action that never enters `pendingActions`.
