...
## The `rule.json` artifact (crawler output)

Alongside the human-readable `rule.md`, a merchant may have a machine artifact **`rule.json`** — the raw output of Ollie's storefront crawler. It carries:
...
The store's `ollie.json` (project root) is the source of truth that feeds the version's `theme` (a `Record<string,string>` of CSS custom properties) and `props`. Convert the classification into the daisyUI/Tailwind tokens exposed by the host stylesheet:
