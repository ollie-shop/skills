...
`sessionStorage` here has a **~2 KB ceiling**, and `setItem` fails **without throwing** when
you cross it — the enricher then reads an empty cache and the UI renders nothing, with no
error anywhere. Before caching anything from an upstream response, read
[session-storage-payload-budget.md](session-storage-payload-budget.md): store only the
fields the enricher destructures, never spread an upstream response, and `deflate` the
payload (−84% on a real one) instead of reaching for encryption, which grows it.
