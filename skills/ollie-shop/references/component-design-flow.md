...
## Design judgement — load it first if it is there

Before Step 1, check whether a `checkout-design` skill is available in this
environment. If it is, load it now: it holds the requirements this flow is held to,
and one of them governs how you ask everything below — lead with a recommendation
and a default, rather than handing the user a list of open questions. If it is not
available, continue; the defaults in each `INSTRUCTIONS.md` stand in for it.

The same applies at the end: if a `checkout-critique` skill is available, run it
against the built component before calling the work done.
...
Once the slot / trigger is resolved, load the target entry's `INSTRUCTIONS.md` (`assets/components/<id>/INSTRUCTIONS.md` or `assets/functions/<id>/INSTRUCTIONS.md`) and follow it — behavior contract, states, semantic structure, SDK touchpoints, "without a design reference" fallback, and antipatterns specific to that pattern. Skip questions already answered by Steps 1–5.

If there is no matching library entry, fall back to `component-anatomy.md` (layout / dev loop), `sdk-guide.md` (hooks), or `hub-functions.md` (handler shape and trigger configuration).
...
- **Suggesting the Figma MCP only after producing code from a screenshot.** Ask up front, when the user first mentions Figma.
...
- **Replicating an HTTP call that does not appear in the HAR.** If the final rawSession shows a state transition and no client-side call in the HAR explains it, the change was made server-side by a hub function. Do not add the call to the component — it will be redundant when the hook works and harmful when the orderForm config rejects it (403). Trust `revalidate: true` on the last blocking request to pick up the side effects.
