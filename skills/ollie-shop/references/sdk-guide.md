...
└── my-component/
    ...
    └── meta.json                    # (pattern A only — see below)

commons/                            # Import-only / shared modules — NOT registered
└── shared-thing/                   # to any slot. Imported via e.g.
    └── index.tsx                   # "../../commons/shared-thing".
```

> **Registration has two supported patterns** (see `references/component-anatomy.md`): (A) per-folder `meta.json` with the component `id`, or (B) a root `ollie.json` `components` map keyed by UUID with `{ path, slot }`. A folder registered by either mechanism is placed into the slot its entry names. A folder NOT registered by either is not a slot component — keep those in a top-level `commons/` folder (sibling of `components/`) so the `components/*/index.tsx` glob never picks them up. This is the fix when two components would otherwise compete for the same slot: render one inside the other and move the inner one to `commons/`.

### `ollie.json` Schema (root)

The config is validated by this shape (extra top-level keys are allowed via passthrough):

```jsonc
{
  "storeId": "<uuid>",              // required
  "versionId": "<uuid>",           // optional — pin a specific version
  "components": {                   // optional — pattern B registration map
    "<component-uuid>": {
      "path": "components/FreeShippingBar",  // required — folder, relative to root
      "slot": "cart_summary_header"          // optional — target slot id
    }
  }
  // passthrough extras also live here: platformStoreId, theme, props, ...
}
```

- The `components` map is optional. Projects using pattern A leave it out and rely on per-folder `meta.json` instead.
- A component entry is exactly `{ path, slot? }` — **no `props`**. A component's props live on its **database record**, set via `ollieshop component create --props` or edited in the Admin/Studio UI.
- The map key is the component UUID from `ollieshop component create`.
- Reserved folder names: `index`, `checkout`.

Props on the component record can be overridden via the Admin without code changes.
