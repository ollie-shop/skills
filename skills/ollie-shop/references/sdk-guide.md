...
- **Payment-tagged components only on payment slots.** `payment-method` and `checkout-summary` are the only slots that receive payment-tag context; placing them elsewhere silently no-ops.
