...
> Audience: front-end engineer (or AI pairing) writing React + TypeScript + **CSS Modules** components for the Ollie Shop checkout. All tokens below are CSS custom properties exposed by the host stylesheet. Always consume via `var(--token)`. **Never** use Tailwind utility classes in these components.
...
Use inline SVG React components (see [Component Anatomy § Shared assets](component-anatomy.md#shared-assets--icons--utils)) with `currentColor` so icons inherit the text color of their context. Sizes (px, inline): `xs 16`, `sm 20`, `md 24` (default), `lg 32`, `xl 40`. Never use Unicode emoji glyphs as icons — they render differently per OS and font fallback.
...
- Language: **pt-BR**. All strings via `useTranslations` (from `next-intl`); never hard-code copy.
...
- ❌ Hard-coded **brand** colors (primary, secondary, accent, success, error, warning, info, base-*). Always `var(--color-*)` — tokens are what let multi-tenant deployments re-theme the same component bundle. Pure neutrals (any grayscale: `rgba(0,0,0,α)`, `rgba(255,255,255,α)`, `rgba(N,N,N,α)` where all channels are equal, or `oklch(L% 0 0 / α)` with chroma 0) are fine for shadows, overlays, and transparent fades because they carry no brand semantic and there is no equivalent token yet (see §Not yet defined).
...
- ❌ Inventing tokens — request the platform team to add them to the host stylesheet **before** consuming.
...
Once decided, request the platform team to add to the host stylesheet and update this contract:
