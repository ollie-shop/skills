...
**How to use:**
1. **[Component Authoring Cheatsheet](component-authoring-cheatsheet.md)** — the lean version you carry while writing (includes the self-check checklist)
...
| **No abbreviations** | Spell it out | `sc`, `qty`, `prod`, `addr` force the reader to decode. Use `salesChannel`, `quantity`, `product`, `address`. |
| **Function names** | Express intent | `useSyncPaymentTerm` tells you what it does. `usePayment` does not. |

### Abbreviations: spell out the local, keep the short name only at the boundary

Don't introduce a variable under a platform abbreviation. The short name is often the real **contract** (a body field, query param, header) — keep it there, but map it to a spelled-out local the moment it enters your code.

```typescript
// ❌ the abbreviation leaks into the logic
const sc = body.sc ?? "";
const scParam = sc ? `&sc=${sc}` : "";

// ✅ spelled-out local; `sc` stays only where it's the actual contract name
const salesChannel = body.sc ?? "";                  // `sc` = VTEX salesChannel
const scParam = salesChannel ? `&sc=${salesChannel}` : "";
```

Conventional throwaway names (`i` in a loop, `err` in a catch) are fine; domain values are not.

---

## Where the self-check lives

The concise checklist you run while writing a component is in `component-authoring-cheatsheet.md` § 11. This file is the deep-dive: the "why" behind each rule and the extra rules for hub functions and actions that don't fit on a cheatsheet. Load this file only when the cheatsheet doesn't answer a specific edge case (hub-function try-catch envelope, action-layer sensitive-data handling, parsing patterns you're unsure how to extract).
