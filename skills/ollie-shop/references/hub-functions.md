...
- **O que faz** — the behavior step by step, including the fail-open vs fail-closed decision and *why* (which side risks worse damage to the customer / order).
- **ENV** — a table of `variable | required? | description`, or "Nenhuma" if the function reads no env.
- **Trigger recomendado** — invocation type + url + expression, mirroring the JSDoc block in `index.ts`. Keep the two in sync.
- **Deploy** — what goes in the zip (typically `index.ts` + `package.json`; note if anything extra is needed).
- **Escopo** — what the function deliberately does **NOT** do (guards against scope creep in review).
- **Testes** — how to run the local test suite for this function.
