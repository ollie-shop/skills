...
> **Writing one → this file. Debugging a deployed one → [`function-debugging.md`](function-debugging.md).** That file owns the triage order, the symptom → cause map, and the platform limits that fail silently (`sessionStorage` ceiling, invocation time budget).
...
Trivial function — one file is enough:

```
functions/<function-name>/
├── index.ts          # exports `handler: CustomFunction` — the build entry
├── index.test.ts     # optional, vitest
├── README.md         # REQUIRED — see "Every function ships a README" below
└── package.json      # see exact shape below
```

`<function-name>` is lowercase-kebab (`cart-enrichment`).

As a function grows, split by responsibility instead of letting `index.ts` swell — same complexity bounds as components (`references/complexity-validation-rules.md`):

```
functions/<function-name>/
├── index.ts          # the handler ONLY: phase guard, cache/short-circuit, response shaping
├── interfaces.ts     # all interfaces/types (Raw* upstream shapes, mapped shape, body)
├── constants.ts      # literals: log prefix, sessionStorage keys, URL paths, defaults
├── utils/
│   └── log.ts        # log()/logError() bound to the function's LOG_PREFIX
├── README.md         # REQUIRED
└── package.json
```

`utils/log.ts` is the one ~every function ends up with. Add other helpers only when the function actually needs them — they're not a fixed scaffold: e.g. a `source.ts` orchestrator for external fetch + mapping, `utils/http.ts` with `getJson`, `utils/validation.ts` with `isHttpUrl`. Don't create files a given function won't use.

The **build doesn't change**: bun bundles from `index.ts` following every relative import and inlines them into a single `index.js` — only that file is zipped. So extra files and a `utils/` subfolder are free; no config, nothing else ships.

Rules of thumb:
- `index.ts` holds the handler and nothing else — no inline parsing, fetching, or literals.
- A literal used more than once (or referenced by the README / another function) → `constants.ts`.
- Anything generic enough to copy into the next function (logging, `getJson`, URL validation) → `utils/`; keep these dependency-free so replicating is a straight copy.
- Don't pre-split a 20-line function — reach for this only when it stops fitting in your head.

### Every function ships a `README.md`

**House rule** — no exceptions. Write it when creating the function and update it in the same change that alters behavior, trigger, or envs. Prescribed sections:

- **One-liner** — the business rule the function enforces, in a sentence.
- **What it does** — the behavior step by step, including the fail-open vs fail-closed decision and *why* (which side risks worse damage to the customer / order).
- **ENV** — a table of `variable | required? | description`, or "None" if the function reads no env.
- **Recommended trigger** — invocation type + url + expression, mirroring the JSDoc block in `index.ts`. Keep the two in sync.
- **Deploy** — what goes in the zip (typically `index.ts` + `package.json`; note if anything extra is needed).
- **Scope** — what the function deliberately does **NOT** do (guards against scope creep in review).
- **Tests** — how to run the local test suite for this function.

The README is the operator-facing contract; the code is the implementation. Reviewers should be able to answer "does this change match what the README promises?" without opening `index.ts`.
...
- **When rewriting a response body, drop `content-length`** — the length changed:
  ...
  ```
- **Session storage:** `globalThis.sessionStorage` is a real key→string store, hydrated from and serialized back to the checkout session each invocation. Use `getItem`/`setItem` to persist small values across the request/response pair or to the storefront. **"Small" is literal: the ceiling is ~2 KB and `setItem` discards silently above it** — no throw, no log, no error in the response. Budget the payload (`patterns/session-storage-payload-budget.md`) and verify the write (`function-debugging.md` §3).
- **Env:** `process.env` is populated from the function's configured env vars for the duration of the call. **Always `.trim()` on read** — the admin does not normalize the saved value, and a leading space is invisible there, survives copy/paste, and produces an upstream error that points at the value rather than the format.
...
### Document the recommended trigger inline

The trigger is configured on the function record, not in code — but **document the recommended trigger as a JSDoc block at the top of `index.ts`**. The hub function is useless without the right trigger, and the trigger expression often encodes the same invariant the handler enforces. Keeping the two together prevents drift.

Concrete example (the in-tree `insurance-sanitizer` function):

```ts
/**
 * Insurance Sanitizer
 *
 * RULE — a seguro is VALID only when the cart is exactly 1 seguro + 1 other item
 * (the insured product), both quantity 1, and the seguro carries the
 * `insurance.referenceItem` attachment linking it to its parent SKU.
 *
 * RECOMMENDED TRIGGER:
 *   url: https://{{account}}.vtexcommercestable.com.br/api/checkout/pub/orderForm/*path
 *   Fire when the cart HAS a seguro AND is NOT in the valid shape (only on success):
 *     $.status >= 200 and $.status < 300
 *     and $count($.body.items[$contains(productCategoryIds,"/4441/") or $contains(productCategoryIds,"/4442/") or $contains(productCategoryIds,"/4443/")]) > 0
 *     and $not(
 *       $count($.body.items[$contains(productCategoryIds,"/4441/")]) = 1
 *       and $count($.body.items) = 2
 *       and $count($.body.items[quantity > 1]) = 0
 *       and $count($.body.items[(...) and $count(attachments[name="insurance.referenceItem"]) = 0]) = 0
 *     )
 */
```
...
- **The expression encodes the invariant.** The handler only runs when the cart breaks the invariant — the hub doesn't even invoke the Lambda on valid carts. The function then self-documents what "valid" means: read the JSDoc, you know the rule.
- **Trigger and handler stay in sync.** When the invariant changes, the JSDoc has to change too, and the next reviewer notices the drift.
- **It is deploy-time guidance, not deploy-time configuration.** Whoever uploads the function reads this block and pastes the expression into the admin form (or passes it to `ollieshop function create --trigger-expression "…"`). It is **not** consumed by the build.
- **Expressions can drill into the body.** Beyond `method` / `headers`, expressions can read `$.body` (and `$.status` for response triggers). Use it to gate the function on the exact bodies that need work — a tight expression keeps cold-start latency off the critical path of every other orderForm call. `$contains`, `$count`, `$not`, and predicate filters (`items[…]`) are all available.

Apply this to every new function: write the trigger first, paste it into the source as JSDoc, then write the handler against the cases the trigger lets through.

---

## Redirecting the customer (303 + `Location`)

Sometimes the right outcome is not "allow" or "reject" — it is "send the customer somewhere else first". Return a `Response` with status `303 See Other` and a `Location` header. The browser follows the redirect on its own — the storefront does not have to recognise a new error code or render a custom modal.

```ts
// Example: a request-phase gate routes the customer to authenticate before retrying.
return new Response("This action requires login.", {
  status: 303,
  headers: { Location: "https://<store>.com.br/login?returnUrl=/checkout" },
});
```

The `Location` URL is **whatever the merchant needs the customer to do next** before the action can be retried. Common targets — pick whichever applies, or invent one:

- a sign-in page with a `returnUrl` back to checkout
- a verification step (liveness, document, OTP)
- a captcha or 3DS challenge
- a regulated-purchase confirmation or age-gate
- a queue / waiting-room page during a flash sale
- any merchant-built remediation flow

The function only decides *whether* to redirect; the target URL is the merchant's call, and a single function can drive several distinct redirect targets from the same verdict (one per branch). Use the redirect outcome when the verdict has a remediation step the user can take. For unconditional blocks use the rejection response contract above.

See `assets/functions/fraud-detector/INSTRUCTIONS.md` for a full multi-outcome example (`request` gate that picks between allow / block / redirect).
