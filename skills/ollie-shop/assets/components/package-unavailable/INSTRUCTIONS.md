...
- Outer wrapper representing the package. The visual tone matches the meaning: neutral for Shape 1, negative (border via `--color-error` + surface tinted via `color-mix(in oklch, var(--color-error) 8%, var(--color-base-100))` or the `--color-alert-error` alert-surface token) for Shape 3. No hardcoded brand colors.
...
- Tokens only from `references/design-contract.md`: `--color-error` / `--color-alert-error` for negative states, `--color-base-content` for text, `--color-base-content` with reduced opacity for muted text, `--color-base-200` for subtle surfaces.
