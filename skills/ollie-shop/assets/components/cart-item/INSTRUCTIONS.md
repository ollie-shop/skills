...
- Use CSS variables for colors from `references/design-contract.md`: `--color-base-content` for primary text, `--color-base-content` with reduced opacity (or `color-mix` with `--color-base-100`) for muted text, `--color-primary` for the CTA/link. No hardcoded brand colors.
