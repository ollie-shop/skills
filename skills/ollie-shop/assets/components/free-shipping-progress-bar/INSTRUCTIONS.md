...
- **Alternative:** `cart_header_full_page` — the canonical position above the cart content. A void slot, so there is nothing to preserve.

Two things to get right on the primary slots. Their default content is the store's price table, so render `{children}` unless you intend to replace the totals. And their reach differs: `checkout_sidebar_totalizer` is **desktop-only** (parent is `hidden lg:flex`), while `cart_sidebar_totalizer` also renders inside the toggled summary modal, which is present on both desktop and mobile — so on desktop one record produces **two bars**, the sidebar one and the modal one.

## Behavior

1. Read the cart subtotal (items total, **excluding** shipping and discounts that aren't shipping-related). Read it from `session.totals.items`, which is an integer in minor units. If a platform does not populate it, fall back to the equivalent totalizer in `rawSession`.
2. Compare the subtotal against a `thresholdCents` value in minor units (passed as a prop; the store sets it on the component record).
3. If `subtotal >= thresholdCents`: render the unlocked state — checkmark + "You unlocked free shipping" copy.
4. Otherwise: render the progress bar at `(subtotal / thresholdCents) * 100`% capped at 100, plus copy showing the remaining amount formatted in the session's currency.
...
- `useCheckoutSession()` — read `session.totals.items` (or `rawSession` totalizers as the platform-specific fallback) and `session.locale` for currency formatting.

## Without a design reference

- Use CSS variables from `references/design-contract.md`: `--color-primary` for the filled portion and the unlock copy, `--color-base-200` for the empty track (subtle surface).
...
- **Don't compute the threshold check against `session.totals.total`.** Total includes shipping and discounts; the threshold gate is supposed to check the *items* subtotal. Gate on `session.totals.items`.
- **Don't render a progress bar when `thresholdCents` is `undefined` or `<= 0`.** It looks broken. Either render nothing or render a static "Free shipping" badge.
